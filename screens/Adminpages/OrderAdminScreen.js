...
const OrderAdminScreen = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const querySnapshotuser = await getDocs(dataCollectionuser);

      const dataArrayuser = querySnapshotuser.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      setUser(dataArrayuser);

      console.log(dataArrayuser);
      ...
      setOrders(dataArray);
      const mergedData = dataArray.map((order) => {
        const user = dataArrayuser.find((user) => user.id === order.userid);
        return { ...order, user };
      });
        ...
        <View style={{ flex: 1, flexDirection: "row", alignItems: "center" }}>
          ...
          >
            ...
            >
              ...
              <Text
                style={{ ...Fonts.blackColor16Medium, textAlign: "left" }}
              >
               {item.Address}
              </Text>
